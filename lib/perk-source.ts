...
import { parsePerkIndependentDamageSnapshot } from "@/lib/perk-preview-catalog";
import {
...
export type PerkChannel = "current" | "preview";
export interface PerkSourceEntry {
  perk: Perk;
  content: string;
...
function parsePreviewChange(value: unknown, channel: PerkChannel, filePath: string): PerkPreviewChange | undefined {
  if (channel === "current" && value === undefined) return undefined;
  if (value !== "new" && value !== "changed" && value !== "existing") {
    throw new Error(`插件 preview_change 必须为 new、changed 或 existing: ${filePath}`);
  }
  return value;
}

export function getPerkSourceEntries(channel: PerkChannel = "current"): PerkSourceEntry[] {
  const PERKS_DATA_DIR = path.join(process.cwd(), channel === "preview" ? "data/perk-preview" : "data/perks");
  if (!fs.existsSync(PERKS_DATA_DIR)) return [];

  const entries: PerkSourceEntry[] = [];
  const identities = new Set<string>();
  ...
  for (let slot = 1; slot <= 4; slot++) {
    ...
    for (const file of files) {
      ...
      if (data.independent_damage_snapshot !== undefined) {
        ...
        parsePerkIndependentDamageSnapshot(data.independent_damage_snapshot);
      }
      const active = getActivePreview();
      if (channel === "preview" ? !active || data.season !== getPreviewSeasonKey(active) : isPreviewSeason(data.season)) {
        throw new Error(`Perk source does not match ${channel} channel: ${filePath}`);
      }
      ...
      const perk: Perk = {
        ...
        previewChange: parsePreviewChange(data.preview_change, channel, filePath),
      ...
      if (identities.has(perk.itemId)) throw new Error(`Duplicate ${channel} perk ItemID: ${perk.itemId}`);
      identities.add(perk.itemId);
      entries.push({ perk, content: body, metadata: data, sourcePath: path.relative(process.cwd(), filePath).replaceAll("\\", "/") });
    }
  }
