...
> 状态：已完成。迁移工具、覆盖结果、异常归属与复核流程见 [`MD/WEAPON-V2-BULK-MIGRATION.md`](./WEAPON-V2-BULK-MIGRATION.md)。
...
- [x] 每个引用都存在于对应 Lock。
- [x] 所有未迁移项都有明确原因和责任分类。
- [x] 没有静默零值、空引用、错误 Item 或跨表引用。
- [x] 所有“ASC 非零但不适用”的来源都有可追溯 override 与原因，不存在按类型批量硬编码的例外。
- [x] 全量页面、搜索、图表和计算器通过回归。
- [x] 迁移工具可重复运行且不会制造无意义 diff。
...
## Task 7.7：清零剩余 V1 排除项

> 状态：已完成。LC 与 TD 各 112 个武器均为 V2；来源证据和审核结果见 [`MD/WEAPON-V2-SOURCE-RECONCILIATION.md`](./WEAPON-V2-SOURCE-RECONCILIATION.md)。本任务没有开始 Task 8。

### 验收结果

- [x] 原 18 把武器的 34 个 V1 表项全部迁移，木葫芦 LC 补充独立恢复来源；全仓达到 `224/224 V2`。
- [x] 90 个首次解析来源与 2 个 1:N 新增来源均有完整身份链证据，剩余 exclude 为 0。
- [x] 葫芦、木葫芦和夜影之逝的恢复 Settlement 不会成为主攻击来源，也不会进入伤害模式投影。
- [x] 能源之影无 ASC 行为、元宵来袭双 ASC、暗夜之殇与火神炎帝技能修正、刺隐 LC/TD 衰减差异均已冻结。
- [x] Lock、迁移快照、试点快照、最终报告与来源对账产物已刷新。
- [x] V1 Resolver、兼容字段、旧消费者清理和 CI 接入均未删除或提前执行。
...
| Task 7.7：清零剩余 V1 排除项 | 已完成 | Task 7.6 | 224/224 V2、92 个新来源审核、exclude 清零 |
| Task 8：删除 V1 并收尾 | 待开始 | Task 7.7 | 单一正式管线、CI 检查、正式文档 |
