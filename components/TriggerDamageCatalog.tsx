...
type PermissionValue = boolean | "不适用";
...
export interface TriggerDamageEntry {
  ...
  element: string;
  critical: PermissionValue | null;
  weakpoint: PermissionValue | null;
...
export function TriggerDamageCatalog({
  entries,
}: {
  entries: readonly TriggerDamageEntry[];
}) {
  return (
    <div className="not-prose my-5 overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900/35">
      {entries.map((entry) => (
        ...
        >
          <header className="grid min-w-0 gap-2 lg:grid-cols-[minmax(12rem,0.8fr)_minmax(20rem,1.4fr)_auto] lg:items-baseline lg:gap-4">
            ...
            <dl className="flex min-w-0 flex-wrap gap-x-4 gap-y-1 text-xs leading-5 text-zinc-500 lg:justify-end">
