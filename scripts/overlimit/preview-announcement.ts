...
export function applyPreviewAnnouncement(catalog: OverlimitCatalog, source = announcement,
  sourcePath = "scripts/overlimit/preview-announcement.json"): OverlimitCatalog {
  if (catalog.season.id !== source.version) return catalog;
  if (!catalog.mapRotation) throw new Error("Missing preview affix schedule");
  const sourcePeriods = catalog.mapRotation.periods;
  catalog.mapRotation.periods = source.periods.map(period => {
  ...
  for (const [id, description] of Object.entries(source.cardDescriptions)) {
    const card = catalog.cards.find(entry => entry.id === id);
    if (!card) throw new Error(`Missing announcement card: ${id}`);
    card.description = description;
  }
  for (const update of source.bondDescriptions) {
  ...
  catalog.season.updatedAt = source.reviewedAt;
  catalog.provenance.note = catalog.season.status === "current"
    ? "正式客户端审定投影；副本开放日期按官方公告，地图羁绊、槽位及已审定数值已复核正式包。服务端概率及未审定数值不作推断。"
    : "S4 预览：副本开放日期按最新官方公告；地图羁绊与已审定数值来自预载配置。公告与配置冲突处单独标注，服务端概率及未审定数值不作推断。";
