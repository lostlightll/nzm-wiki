...
test("the committed Lock is canonical and internally complete", () => {
  ...
  assert.equal(lock.sources.lc.modifiers.row_count, 3675);
  assert.equal(lock.sources.lc.attribute_descriptions.row_count, 188);
  assert.equal(Object.keys(lock.attribute_descriptions.lc).length, 188);
  assert.deepEqual(
