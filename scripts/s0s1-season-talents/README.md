...
`export-taboo-script.ps1 -CliAssembly <installed FModel.Cli.dll> -Profile <private game profile>` delegates to the shared bytecode exporter in a PowerShell host compatible with the installed CLI's .NET runtime (currently .NET 10). It enables `ReadScriptData` and writes a new ignored `kismet/<dataset>/<snapshot>/` directory with a manifest. `-Dataset` defaults to `unknown`; it labels the output and does not select the game installation. See [Kismet evidence](../../docs/workflows/kismet-evidence.md). It does not install/build tools, amend refs, or print the private profile. The reflected CLI API is version-specific; failures require inspection, not fallback to signature-only JSON.
