...
经实测或执行配置确认的射击间隔差异使用 `overrides.asc.fire_interval`，连发数量与段内间隔分别使用 `overrides.asc.sub_fire_count` 和 `overrides.asc.sub_fire_interval`。Resolver 按继承链顺序应用覆盖并保留每一步历史；射击间隔覆盖后重新计算 RPM，最终间隔为零时 RPM 明确为 `unavailable`。兼容 `fire_interval` 与最终有效值比较，不与覆盖前 ASC 值比较。
